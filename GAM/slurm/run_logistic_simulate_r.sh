#!/bin/bash
#SBATCH --job-name=gam_logistic_sim_r
...
# Submit from the project root: sbatch public/slurm/run_logistic_simulate_r.sh
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
mkdir -p "$ROOT/logs"

module load R/4.3.1   # adjust R version as needed

Rscript "$ROOT/public/simulate_logistic_r.R"
